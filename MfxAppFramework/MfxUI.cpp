...
void MicroFlakeX::MfxUI::MfxRegMessages()
{
    ...
    UI_CON_RECV_MSG(MfxUI_Message_PaintTexture);
    UI_CON_RECV_MSG(MfxUI_Message_PaintImage);
    ...
    UI_REG_MSG(MfxUI_Message_PaintTexture, MfxUI, __OnPaintTexture, myCoverFloor);
    UI_REG_MSG(MfxUI_Message_PaintImage, MfxUI, __OnPaintImage, myCoverFloor);
...
MicroFlakeX::MfxUI::~MfxUI()
{
    ...
    MfxComRelease(&myDCRenderTarget);
...
MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnCreate(WPARAM wParam, LPARAM lParam)
{
    ...
    myDCRenderTarget = MfxGetID2D1DCRenderTarget(myBackDC, GdipRect(0, 0, myRect.X, myRect.Y));

    myBackImage = new MfxTexture;
    myBackImage->SetRect(GdipRect(0, 0, t_Size.Width, t_Size.Height));
    myBackImage->FromFile(L"D:\\image\\rect.jpg");
    myBackImage->SetRenderTarget(myDCRenderTarget);
...
MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnSize(WPARAM wParam, LPARAM lParam)
{
    ...
    rc.top = 0;
    rc.left =0;
    rc.right = myRect.Width;
    rc.bottom = myRect.Height;
    myDCRenderTarget->BindDC(myBackDC, &rc);
...
MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnPaint(WPARAM wParam, LPARAM lParam)
{
    ...
    __OnPaintTexture(NULL, NULL);

    __OnPaintImage(NULL, NULL);
...
MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnPaintTexture(WPARAM wParam, LPARAM lParam)
{
    MfxCodeLock t_CodeLock(this);
    myDCRenderTarget->BeginDraw();
    if (myBackImage)
    {
        myBackImage->Draw();
    }
    this->SendMessageToControls(MfxUI_Message_PaintTexture, wParam, lParam, true);
    myDCRenderTarget->EndDraw();

    return MfxFine;
}

MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnPaintImage(WPARAM wParam, LPARAM lParam)
{
    MfxCodeLock t_CodeLock(this);
    //BitBlt(myMaskDC, 0, 0, myRect.Width, myRect.Height, myBackDC, 0, 0, SRCCOPY);
    //宸戦紙崙Mask

    this->SendMessageToControls(MfxUI_Message_PaintImage, wParam, lParam, true);
    if (myMaskImage)
    {
        myMaskImage->Draw();
    }
...
MicroFlakeX::MfxReturn MicroFlakeX::MfxUI::__OnControlInsert(WPARAM wParam, LPARAM lParam)
{
    ...
    if (wParam)
    {
        if (myControlSet.insert((MfxControl*)wParam).second)
        {
            ...
            MfxUI_Paper_Value t_PaperValue(this, myWnd, myBackDC, myDCRenderTarget);
