...
namespace MicroFlakeX
{
	...
	{
		MfxUI_Paper_Value(MfxUI* setUI, HWND setWnd, HDC setBack, ID2D1RenderTarget* setRender)
		{
			myUI = setUI;
			myWnd = setWnd;
			myBackDC = setBack;
...
namespace MicroFlakeX
{
	...
	enum MfxUI_Message
	{
		...
		MfxUI_Message_PaintTexture = MfxUI_MessageBegin - 3,
		MfxUI_Message_PaintImage = MfxUI_MessageBegin - 4,
...
#define UI_ADD_TIMER(timerID, delay, recvFunc)\
	...
	protected:
		HWND myWnd;
		GdipRect myRect;
		HDC myWndDc, myBackDC;
		MfxTexture* myBackImage, * myMaskImage;
		GdipGraphics* myWndGraphics;
		ID2D1DCRenderTarget* myDCRenderTarget;
	public:
...
#define UI_REG_MSG(Msg, myClass, FuncName, Floor)\
	...
	protected:
		...
		MfxReturn __OnPaintTexture(WPARAM wParam, LPARAM lParam);
		MfxReturn __OnPaintImage(WPARAM wParam, LPARAM lParam);
